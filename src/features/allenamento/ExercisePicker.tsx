...
import { exerciseEmoji } from './muscleEmoji';
      ...
      <div className="space-y-1.5 pb-2">
        {list.map((e) => (
          <button key={e.id} onClick={() => onAdd(e)} className="w-full flex items-center gap-3 p-3 rounded-2xl bg-section text-left active:opacity-80">
            <span className="h-9 w-9 rounded-xl flex items-center justify-center text-[18px] flex-shrink-0" style={{ background: 'color-mix(in srgb, var(--c-primary) 12%, var(--c-card))', boxShadow: 'inset 0 0 0 1px color-mix(in srgb, var(--c-primary) 28%, transparent)' }} aria-hidden>{exerciseEmoji(e.id, e.muscle)}</span>
