# GUIDA COMPLETA — Pubblicare Vyta 1.2 (build 35) con i widget
...
48. Lascia le opzioni di default (Upload symbols, Manage signing automatically)
    → **Next** → **Upload**.
49. Attendi "Upload successful". Il build **35** ora è in elaborazione su App
...
52. **Build**: scorri fino alla sezione *Build* → clicca **(+)** o **Add Build**
    → seleziona il **build 35** (quando ha finito di elaborare; se non lo vedi,
    aspetta e ricarica).
...
- **Il build 35 non appare in App Store Connect** → è ancora "in elaborazione",
